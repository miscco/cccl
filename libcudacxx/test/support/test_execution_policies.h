...
#define EXECUTION_POLICY_SFINAE_TEST(function)                                 \
  ...
  struct sfinae_test_##function##_impl : cuda::std::true_type {};              \
                                                                               \
  template <class... Args>                                                     \
  struct sfinae_test_##function##_impl<                                        \
      cuda::std::void_t<decltype(                                              \
          cuda::std::function(cuda::std::declval<Args>()...))>,                \
      Args...> : cuda::std::false_type {};                                     \
                                                                               \
  template <class... Args>                                                     \
  constexpr bool sfinae_test_##function =                                      \
      sfinae_test_##function##_impl<void, Args...>::value;
...
struct Bool {
  bool b_ = false;
  Bool() = default;
  __host__ __device__  Bool(bool b) : b_(b) {}
  __host__ __device__  operator bool&() { return b_; }
};
