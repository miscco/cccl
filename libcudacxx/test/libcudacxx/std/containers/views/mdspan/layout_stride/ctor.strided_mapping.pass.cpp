...
__host__ __device__ constexpr auto get_strides(FromE src_exts)
{
  ...
  cuda::std::array<size_t, FromE::rank()> strides{};
  size_t stride = 2;
...
template <bool implicit, class FromL, class ToE, class FromE, cuda::std::enable_if_t<implicit, int> = 0>
__host__ __device__ constexpr void test_conversion(FromE src_exts)
{
  using To   = cuda::std::layout_stride::mapping<ToE>;
  using From = typename FromL::template mapping<FromE>;
