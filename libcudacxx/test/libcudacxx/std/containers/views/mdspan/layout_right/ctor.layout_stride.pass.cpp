...
__host__ __device__ constexpr auto get_strides(FromE src_exts)
{
...
template <class FromE, cuda::std::enable_if_t<(FromE::rank() == 0), int> = 0>
__host__ __device__ constexpr auto get_strides(FromE)
{
  return cuda::std::array<typename FromE::index_type, FromE::rank()>{};
}

template <bool implicit, class ToE, class FromE>
__host__ __device__ constexpr void test_conversion(FromE src_exts)
{
  using To           = cuda::std::layout_left::mapping<ToE>;
  using From         = cuda::std::layout_stride::mapping<FromE>;
