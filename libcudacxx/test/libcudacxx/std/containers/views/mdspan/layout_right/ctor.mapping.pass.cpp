...
template <bool implicit, class ToE, class FromE, cuda::std::enable_if_t<implicit, int> = 0>
__host__ __device__ constexpr void test_conversion(FromE src_exts)
{
  using To   = cuda::std::layout_right::mapping<ToE>;
  using From = cuda::std::layout_right::mapping<FromE>;
  From src(src_exts);
