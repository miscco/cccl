...
template <class FromE, cuda::std::enable_if_t<(FromE::rank() > 0), int> = 0>
__host__ __device__ constexpr cuda::std::array<typename FromE::index_type, FromE::rank()> get_strides(FromE src_exts)
{
  cuda::std::array<typename FromE::index_type, FromE::rank()> strides{};
  strides[0] = 1;
...
template <class FromE, cuda::std::enable_if_t<(FromE::rank() == 0), int> = 0>
__host__ __device__ constexpr cuda::std::array<typename FromE::index_type, FromE::rank()> get_strides(FromE)
{
  return {};
}
...
template <bool implicit,
          class ToE,
          class FromE,
          class To   = cuda::std::layout_left::mapping<ToE>,
          class From = cuda::std::layout_stride::mapping<FromE>>
__host__ __device__ constexpr void test_conversion(FromE src_exts)
{
  const cuda::std::array<typename FromE::index_type, FromE::rank()> strides = get_strides(src_exts);
