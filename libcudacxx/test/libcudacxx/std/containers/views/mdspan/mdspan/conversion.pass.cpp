...
construct_from_mds(const FromH& handle, const FromL& layout, const FromE& exts, const FromA& acc)
{
  using ToMDS   = cuda::std::mdspan<typename ToA::element_type, ToE, ToL, ToA>;
  using FromMDS = cuda::std::mdspan<typename FromA::element_type, FromE, FromL, FromA>;
