// -*- C++ -*-
//===----------------------------------------------------------------------===//
//
// Part of the LLVM Project, under the Apache License v2.0 with LLVM Exceptions.
// See https://llvm.org/LICENSE.txt for license information.
// SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception
//
//                        Kokkos v. 4.0
//       Copyright (2022) National Technology & Engineering
//               Solutions of Sandia, LLC (NTESS).
//
// Under the terms of Contract DE-NA0003525 with NTESS,
// the U.S. Government retains certain rights in this software.
//
//===---------------------------------------------------------------------===//

#ifndef _LIBCUDACXX___MDSPAN_EXTENTS_HPP
#define _LIBCUDACXX___MDSPAN_EXTENTS_HPP

#include <cuda/std/detail/__config>

#if defined(_CCCL_IMPLICIT_SYSTEM_HEADER_GCC)
#  pragma GCC system_header
#elif defined(_CCCL_IMPLICIT_SYSTEM_HEADER_CLANG)
#  pragma clang system_header
#elif defined(_CCCL_IMPLICIT_SYSTEM_HEADER_MSVC)
#  pragma system_header
#endif // no system header

#include <cuda/std/__concepts/__concept_macros.h>
#include <cuda/std/__mdspan/concepts.h>
#include <cuda/std/__type_traits/common_type.h>
#include <cuda/std/__type_traits/fold.h>
#include <cuda/std/__type_traits/integral_constant.h>
#include <cuda/std/__type_traits/is_convertible.h>
#include <cuda/std/__type_traits/is_nothrow_constructible.h>
#include <cuda/std/__type_traits/is_same.h>
#include <cuda/std/__type_traits/make_unsigned.h>
#include <cuda/std/__utility/integer_sequence.h>
#include <cuda/std/__utility/unreachable.h>
#include <cuda/std/array>
#include <cuda/std/concepts>
#include <cuda/std/cstddef>
#include <cuda/std/detail/libcxx/include/__assert>
#include <cuda/std/limits>
#include <cuda/std/span>

_CCCL_PUSH_MACROS

#if _CCCL_STD_VER >= 2014

_LIBCUDACXX_BEGIN_NAMESPACE_STD

namespace __mdspan_detail
{

// ------------------------------------------------------------------
// ------------ __static_array --------------------------------------
// ------------------------------------------------------------------
// array like class which provides an array of static values with get
template <class _Tp, _Tp... _Values>
struct __static_array
{
public:
  _LIBCUDACXX_INLINE_VISIBILITY static constexpr size_t __size()
  {
    return sizeof...(_Values);
  }

  _LIBCUDACXX_INLINE_VISIBILITY static constexpr _Tp __get(size_t __index) noexcept
  {
    constexpr array<_Tp, sizeof...(_Values)> __array = {_Values...};
    return __array[__index];
  }

  template <size_t _Index>
  _LIBCUDACXX_INLINE_VISIBILITY static constexpr _Tp __get()
  {
    return __get(_Index);
  }
};

// ------------------------------------------------------------------
// ------------ __possibly_empty_array  -----------------------------
// ------------------------------------------------------------------

// array like class which provides get function and operator [], and
// has a specialization for the size 0 case.
// This is needed to make the __maybe_static_array be truly empty, for
// all static values.

template <class _Tp, size_t _Size>
struct __possibly_empty_array
{
  _Tp __vals_[_Size];
  _LIBCUDACXX_INLINE_VISIBILITY constexpr _Tp& operator[](size_t __index)
  {
    return __vals_[__index];
  }
  _LIBCUDACXX_INLINE_VISIBILITY constexpr const _Tp& operator[](size_t __index) const
  {
    return __vals_[__index];
  }
};

template <class _Tp>
struct __possibly_empty_array<_Tp, 0>
{
  _LIBCUDACXX_INLINE_VISIBILITY constexpr _Tp& operator[](size_t)
  {
    _LIBCUDACXX_UNREACHABLE();
  }
  _LIBCUDACXX_INLINE_VISIBILITY constexpr const _Tp& operator[](size_t) const
  {
    _LIBCUDACXX_UNREACHABLE();
  }
};

// ------------------------------------------------------------------
// ------------ static_partial_sums ---------------------------------
// ------------------------------------------------------------------

// Provides a compile time partial sum one can index into

template <size_t... _Values>
struct __static_partial_sums
{
  _LIBCUDACXX_INLINE_VISIBILITY static constexpr array<size_t, sizeof...(_Values)> __static_partial_sums_impl()
  {
    array<size_t, sizeof...(_Values)> __values{_Values...};
    array<size_t, sizeof...(_Values)> __partial_sums{{}};
    size_t __running_sum = 0;
    for (int __i = 0; __i != sizeof...(_Values); ++__i)
    {
      __partial_sums[__i] = __running_sum;
      __running_sum += __values[__i];
    }
    return __partial_sums;
  }

  _LIBCUDACXX_INLINE_VISIBILITY static constexpr size_t __get(size_t __index)
  {
    constexpr array<size_t, sizeof...(_Values)> __result = __static_partial_sums_impl();
    return __result[__index];
  }
};

// ------------------------------------------------------------------
// ------------ __maybe_static_array --------------------------------
// ------------------------------------------------------------------

// array like class which has a mix of static and runtime values but
// only stores the runtime values.
// The type of the static and the runtime values can be different.
// The position of a dynamic value is indicated through a tag value.
template <class _TDynamic, class _TStatic, _TStatic _DynTag, _TStatic... _Values>
struct __maybe_static_array
{
  static_assert(is_convertible<_TStatic, _TDynamic>::value,
                "__maybe_static_array: _TStatic must be convertible to _TDynamic");
  static_assert(is_convertible<_TDynamic, _TStatic>::value,
                "__maybe_static_array: _TDynamic must be convertible to _TStatic");

private:
  // Static values member
  static constexpr size_t __size_         = sizeof...(_Values);
  static constexpr size_t __size_dynamic_ = _CCCL_FOLD_PLUS(size_t(0), static_cast<size_t>(_Values == _DynTag));
  using _StaticValues                     = __static_array<_TStatic, _Values...>;
  using _DynamicValues                    = __possibly_empty_array<_TDynamic, __size_dynamic_>;

  // Dynamic values member
  _CCCL_NO_UNIQUE_ADDRESS _DynamicValues __dyn_vals_;

  // static mapping of indices to the position in the dynamic values array
  using _DynamicIdxMap = __static_partial_sums<static_cast<size_t>(_Values == _DynTag)...>;

  template <size_t... Indices>
  _LIBCUDACXX_INLINE_VISIBILITY static constexpr _DynamicValues __zeros(index_sequence<Indices...>) noexcept
  {
    return _DynamicValues{((void) Indices, 0)...};
  }

public:
  _LIBCUDACXX_INLINE_VISIBILITY constexpr __maybe_static_array() noexcept
      : __dyn_vals_{__zeros(make_index_sequence<__size_dynamic_>())}
  {}

  // constructors from dynamic values only -- this covers the case for rank() == 0
  _LIBCUDACXX_TEMPLATE(class... _DynVals)
  _LIBCUDACXX_REQUIRES((sizeof...(_DynVals) == __size_dynamic_) && (!__all<__is_std_span<_DynVals>...>::value))
  _LIBCUDACXX_INLINE_VISIBILITY constexpr __maybe_static_array(_DynVals... __vals) noexcept
      : __dyn_vals_{static_cast<_TDynamic>(__vals)...}
  {}

  _LIBCUDACXX_TEMPLATE(class _Tp, size_t _Size)
  _LIBCUDACXX_REQUIRES((_Size == __size_dynamic_) _LIBCUDACXX_AND(_Size == 0))
  _LIBCUDACXX_INLINE_VISIBILITY constexpr __maybe_static_array(span<_Tp, _Size>) noexcept {}

  _LIBCUDACXX_TEMPLATE(class _Tp, size_t _Size)
  _LIBCUDACXX_REQUIRES((_Size == __size_dynamic_) _LIBCUDACXX_AND(_Size > 0))
  _LIBCUDACXX_INLINE_VISIBILITY constexpr __maybe_static_array(span<_Tp, _Size> __vals) noexcept
  {
    for (size_t __i = 0; __i < _Size; __i++)
    {
      __dyn_vals_[__i] = static_cast<_TDynamic>(__vals[__i]);
    }
  }

  // constructors from all values -- here rank will be greater than 0
  _LIBCUDACXX_TEMPLATE(class... _DynVals)
  _LIBCUDACXX_REQUIRES((sizeof...(_DynVals) != __size_dynamic_) && (!__all<__is_std_span<_DynVals>...>::value))
  _LIBCUDACXX_INLINE_VISIBILITY constexpr __maybe_static_array(_DynVals... __vals)
  {
    static_assert((sizeof...(_DynVals) == __size_), "Invalid number of values.");
    _TDynamic __values[__size_] = {static_cast<_TDynamic>(__vals)...};
    for (size_t __i = 0; __i < __size_; __i++)
    {
      _TStatic __static_val = _StaticValues::__get(__i);
      if (__static_val == _DynTag)
      {
        __dyn_vals_[_DynamicIdxMap::__get(__i)] = __values[__i];
      }
      else
      {
        // Not catching this could lead to out of bounds errors later
        // e.g. using my_mdspan_t = mdspan<int, extents<int, 10>>; my_mdspan_t = m(new int[5], 5);
        // Right-hand-side construction looks ok with allocation and size matching,
        // but since (potentially elsewhere defined) my_mdspan_t has static size m now thinks its range is 10 not 5
        _LIBCUDACXX_ASSERT_VALID_ELEMENT_ACCESS(
          __values[__i] == static_cast<_TDynamic>(__static_val),
          "extents construction: mismatch of provided arguments with static extents.");
      }
    }
  }

  _LIBCUDACXX_TEMPLATE(class _Tp, size_t _Size)
  _LIBCUDACXX_REQUIRES((_Size != __size_dynamic_))
  _LIBCUDACXX_INLINE_VISIBILITY constexpr __maybe_static_array(span<_Tp, _Size> __vals)
  {
    static_assert((_Size == __size_) || (__size_ == dynamic_extent));
    for (size_t __i = 0; __i < __size_; __i++)
    {
      _TStatic __static_val = _StaticValues::__get(__i);
      if (__static_val == _DynTag)
      {
        __dyn_vals_[_DynamicIdxMap::__get(__i)] = static_cast<_TDynamic>(__vals[__i]);
      }
      else
      {
        // Not catching this could lead to out of bounds errors later
        // e.g. using my_mdspan_t = mdspan<int, extents<int, 10>>; my_mdspan_t = m(new int[N], span<int,1>(&N));
        // Right-hand-side construction looks ok with allocation and size matching,
        // but since (potentially elsewhere defined) my_mdspan_t has static size m now thinks its range is 10 not N
        _LIBCUDACXX_ASSERT_VALID_ELEMENT_ACCESS(
          static_cast<_TDynamic>(__vals[__i]) == static_cast<_TDynamic>(__static_val),
          "extents construction: mismatch of provided arguments with static extents.");
      }
    }
  }

  // access functions
  _LIBCUDACXX_INLINE_VISIBILITY static constexpr _TStatic __static_value(size_t __i) noexcept
  {
    _LIBCUDACXX_ASSERT_VALID_ELEMENT_ACCESS(__i < __size_, "extents access: index must be less than rank");
    return _StaticValues::__get(__i);
  }

  _LIBCUDACXX_INLINE_VISIBILITY constexpr _TDynamic __value(size_t __i) const
  {
    _LIBCUDACXX_ASSERT_VALID_ELEMENT_ACCESS(__i < __size_, "extents access: index must be less than rank");
    _TStatic __static_val = _StaticValues::__get(__i);
    return __static_val == _DynTag ? __dyn_vals_[_DynamicIdxMap::__get(__i)] : static_cast<_TDynamic>(__static_val);
  }
  _LIBCUDACXX_INLINE_VISIBILITY constexpr _TDynamic operator[](size_t __i) const
  {
    _LIBCUDACXX_ASSERT_VALID_ELEMENT_ACCESS(__i < __size_, "extents access: index must be less than rank");
    return __value(__i);
  }

  // observers
  _LIBCUDACXX_INLINE_VISIBILITY static constexpr size_t __size()
  {
    return __size_;
  }
  _LIBCUDACXX_INLINE_VISIBILITY static constexpr size_t __size_dynamic()
  {
    return __size_dynamic_;
  }
};

template <class _To, class _From>
static constexpr bool __potentially_narrowing =
  static_cast<make_unsigned_t<_To>>((numeric_limits<_To>::max)())
  < static_cast<make_unsigned_t<_From>>((numeric_limits<_From>::max)());

// Function to check whether a value is representable as another type
// value must be a positive integer otherwise returns false
// if _From is not an integral, we just check positivity
_LIBCUDACXX_TEMPLATE(class _To, class _From)
_LIBCUDACXX_REQUIRES(integral<_To> _LIBCUDACXX_AND integral<_From> _LIBCUDACXX_AND _CCCL_TRAIT(is_signed, _From)
                       _LIBCUDACXX_AND(!__potentially_narrowing<_To, _From>))
_LIBCUDACXX_INLINE_VISIBILITY constexpr bool __is_representable_as(_From __value)
{
  return __value >= 0;
}

_LIBCUDACXX_TEMPLATE(class _To, class _From)
_LIBCUDACXX_REQUIRES(integral<_To> _LIBCUDACXX_AND integral<_From> _LIBCUDACXX_AND _CCCL_TRAIT(is_signed, _From)
                       _LIBCUDACXX_AND __potentially_narrowing<_To, _From>)
_LIBCUDACXX_INLINE_VISIBILITY constexpr bool __is_representable_as(_From __value)
{
  using _To_u   = make_unsigned_t<_To>;
  using _From_u = make_unsigned_t<_From>;
  if (__value < 0)
  {
    return false;
  }
  return static_cast<_To_u>((numeric_limits<_To>::max)()) >= static_cast<_From_u>(__value);
}

_LIBCUDACXX_TEMPLATE(class _To, class _From)
_LIBCUDACXX_REQUIRES(integral<_To> _LIBCUDACXX_AND integral<_From> _LIBCUDACXX_AND(!_CCCL_TRAIT(is_signed, _From))
                       _LIBCUDACXX_AND(!__potentially_narrowing<_To, _From>))
_LIBCUDACXX_INLINE_VISIBILITY constexpr bool __is_representable_as(_From __value)
{
  return true;
}

_LIBCUDACXX_TEMPLATE(class _To, class _From)
_LIBCUDACXX_REQUIRES(integral<_To> _LIBCUDACXX_AND integral<_From> _LIBCUDACXX_AND(!_CCCL_TRAIT(is_signed, _From))
                       _LIBCUDACXX_AND __potentially_narrowing<_To, _From>)
_LIBCUDACXX_INLINE_VISIBILITY constexpr bool __is_representable_as(_From __value)
{
  using _To_u   = make_unsigned_t<_To>;
  using _From_u = make_unsigned_t<_From>;
  return static_cast<_To_u>((numeric_limits<_To>::max)()) >= static_cast<_From_u>(__value);
}

_LIBCUDACXX_TEMPLATE(class _To, class _From)
_LIBCUDACXX_REQUIRES(integral<_To> _LIBCUDACXX_AND(!integral<_From>) _LIBCUDACXX_AND _CCCL_TRAIT(is_signed, _To))
_LIBCUDACXX_INLINE_VISIBILITY constexpr bool __is_representable_as(_From __value)
{
  return static_cast<_To>(__value) >= 0;
}

_LIBCUDACXX_TEMPLATE(class _To, class _From)
_LIBCUDACXX_REQUIRES(integral<_To> _LIBCUDACXX_AND(!integral<_From>) _LIBCUDACXX_AND(!_CCCL_TRAIT(is_signed, _To)))
_LIBCUDACXX_INLINE_VISIBILITY constexpr bool __is_representable_as(_From __value)
{
  return true;
}

_LIBCUDACXX_TEMPLATE(class _To, class... _From)
_LIBCUDACXX_REQUIRES(integral<_To>)
_LIBCUDACXX_INLINE_VISIBILITY constexpr bool __are_representable_as(_From... __values)
{
  return __all<__mdspan_detail::__is_representable_as<_To>(__values)...>::value;
}

_LIBCUDACXX_TEMPLATE(class _To, class _From, size_t _Size)
_LIBCUDACXX_REQUIRES(integral<_To>)
_LIBCUDACXX_INLINE_VISIBILITY constexpr bool __are_representable_as(span<_From, _Size> __values)
{
  for (size_t __i = 0; __i < _Size; __i++)
  {
    if (!__mdspan_detail::__is_representable_as<_To>(__values[__i]))
    {
      return false;
    }
  }
  return true;
}

} // namespace __mdspan_detail

// ------------------------------------------------------------------
// ------------ extents ---------------------------------------------
// ------------------------------------------------------------------

// Class to delegate between the different (non-)explicit constructors
struct __extent_delegate_tag
{};

// Class to describe the extents of a multi dimensional array.
// Used by mdspan, mdarray and layout mappings.
// See ISO C++ standard [mdspan.extents]
template <class _IndexType, size_t... _Extents>
class extents
{
public:
  // typedefs for integral types used
  using index_type = _IndexType;
  using size_type  = make_unsigned_t<index_type>;
  using rank_type  = size_t;

  static_assert(is_integral<index_type>::value && !is_same<index_type, bool>::value,
                "extents::index_type must be a signed or unsigned integer type");
  static_assert(
    __all<(__mdspan_detail::__is_representable_as<index_type>(_Extents) || (_Extents == dynamic_extent))...>::value,
    "extents ctor: arguments must be representable as index_type and nonnegative");

private:
  static constexpr rank_type __rank_ = sizeof...(_Extents);
  static constexpr rank_type __rank_dynamic_ =
    _CCCL_FOLD_PLUS(rank_type(0), (static_cast<rank_type>(_Extents == dynamic_extent)));

  // internal storage type using __maybe_static_array
  using _Values = __mdspan_detail::__maybe_static_array<_IndexType, size_t, dynamic_extent, _Extents...>;
  _CCCL_NO_UNIQUE_ADDRESS _Values __vals_;

public:
  // [mdspan.extents.obs], observers of multidimensional index space
  _LIBCUDACXX_INLINE_VISIBILITY static constexpr rank_type rank() noexcept
  {
    return __rank_;
  }
  _LIBCUDACXX_INLINE_VISIBILITY static constexpr rank_type rank_dynamic() noexcept
  {
    return __rank_dynamic_;
  }

  _LIBCUDACXX_INLINE_VISIBILITY constexpr index_type extent(rank_type __r) const noexcept
  {
    return __vals_.__value(__r);
  }
  _LIBCUDACXX_INLINE_VISIBILITY static constexpr size_t static_extent(rank_type __r) noexcept
  {
    return _Values::__static_value(__r);
  }

  // [mdspan.extents.cons], constructors
  constexpr extents() noexcept = default;

  template <class... _OtherIndexTypes>
  static constexpr bool __all_convertible_to_index_type =
    _CCCL_FOLD_AND(_CCCL_TRAIT(is_convertible, _OtherIndexTypes, index_type))
    && _CCCL_FOLD_AND(_CCCL_TRAIT(is_nothrow_constructible, index_type, _OtherIndexTypes));

  // Construction from just dynamic or all values.
  // Precondition check is deferred to __maybe_static_array constructor
  _LIBCUDACXX_TEMPLATE(class... _OtherIndexTypes)
  _LIBCUDACXX_REQUIRES((sizeof...(_OtherIndexTypes) == __rank_ || sizeof...(_OtherIndexTypes) == __rank_dynamic_)
                         _LIBCUDACXX_AND __all_convertible_to_index_type<_OtherIndexTypes...>)
  _LIBCUDACXX_INLINE_VISIBILITY constexpr explicit extents(_OtherIndexTypes... __dynvals) noexcept
      : __vals_(static_cast<index_type>(__dynvals)...)
  {
    // Not catching this could lead to out of bounds errors later
    // e.g. mdspan m(ptr, dextents<char, 1>(200u)); leads to an extent of -56 on m
    _LIBCUDACXX_ASSERT_VALID_ELEMENT_ACCESS(
      __mdspan_detail::__are_representable_as<index_type>(__dynvals...),
      "extents ctor: arguments must be representable as index_type and nonnegative");
  }

  template <class _OtherIndexType>
  static constexpr bool __is_convertible_to_index_type =
    _CCCL_TRAIT(is_convertible, const _OtherIndexType&, index_type)
    && _CCCL_TRAIT(is_nothrow_constructible, index_type, const _OtherIndexType&);

  _LIBCUDACXX_TEMPLATE(class _OtherIndexType, size_t _Size)
  _LIBCUDACXX_REQUIRES((_Size == __rank_dynamic_) _LIBCUDACXX_AND __is_convertible_to_index_type<_OtherIndexType>)
  _LIBCUDACXX_INLINE_VISIBILITY constexpr extents(const array<_OtherIndexType, _Size>& __exts) noexcept
      : extents(span<const _OtherIndexType, _Size>(__exts))
  {}

  _LIBCUDACXX_TEMPLATE(class _OtherIndexType, size_t _Size)
  _LIBCUDACXX_REQUIRES((_Size == __rank_) _LIBCUDACXX_AND(_Size != __rank_dynamic_)
                         _LIBCUDACXX_AND __is_convertible_to_index_type<_OtherIndexType>)
  _LIBCUDACXX_INLINE_VISIBILITY explicit constexpr extents(const array<_OtherIndexType, _Size>& __exts) noexcept
      : extents(span<const _OtherIndexType, _Size>(__exts))
  {}

  _LIBCUDACXX_TEMPLATE(class _OtherIndexType, size_t _Size)
  _LIBCUDACXX_REQUIRES((_Size == __rank_dynamic_) _LIBCUDACXX_AND __is_convertible_to_index_type<_OtherIndexType>)
  _LIBCUDACXX_INLINE_VISIBILITY constexpr extents(span<_OtherIndexType, _Size> __exts) noexcept
      : __vals_(__exts)
  {
    // Not catching this could lead to out of bounds errors later
    // e.g. array a{200u}; mdspan<int, dextents<char,1>> m(ptr, extents(span<unsigned,1>(a))); leads to an extent of -56
    // on m
    _LIBCUDACXX_ASSERT_VALID_ELEMENT_ACCESS(
      __mdspan_detail::__are_representable_as<index_type>(__exts),
      "extents ctor: arguments must be representable as index_type and nonnegative");
  }

  _LIBCUDACXX_TEMPLATE(class _OtherIndexType, size_t _Size)
  _LIBCUDACXX_REQUIRES((_Size != __rank_dynamic_) _LIBCUDACXX_AND(_Size == __rank_)
                         _LIBCUDACXX_AND __is_convertible_to_index_type<_OtherIndexType>)
  _LIBCUDACXX_INLINE_VISIBILITY explicit constexpr extents(span<_OtherIndexType, _Size> __exts) noexcept
      : __vals_(__exts)
  {
    // Not catching this could lead to out of bounds errors later
    // e.g. array a{200u}; mdspan<int, dextents<char,1>> m(ptr, extents(span<unsigned,1>(a))); leads to an extent of -56
    // on m
    _LIBCUDACXX_ASSERT_VALID_ELEMENT_ACCESS(
      __mdspan_detail::__are_representable_as<index_type>(__exts),
      "extents ctor: arguments must be representable as index_type and nonnegative");
  }

private:
  // Function to construct extents storage from other extents.
  _LIBCUDACXX_TEMPLATE(size_t _DynCount, size_t _Idx, class _OtherExtents, class... _DynamicValues)
  _LIBCUDACXX_REQUIRES((_Idx < __rank_) _LIBCUDACXX_AND(static_extent(_Idx) == dynamic_extent))
  _LIBCUDACXX_INLINE_VISIBILITY constexpr _Values __construct_vals_from_extents(
    integral_constant<size_t, _DynCount>,
    integral_constant<size_t, _Idx>,
    const _OtherExtents& __exts,
    _DynamicValues... __dynamic_values) noexcept
  {
    return __construct_vals_from_extents(
      integral_constant<size_t, _DynCount + 1>(),
      integral_constant<size_t, _Idx + 1>(),
      __exts,
      __dynamic_values...,
      __exts.extent(_Idx));
  }

  // Function to construct extents storage from other extents.
  _LIBCUDACXX_TEMPLATE(size_t _DynCount, size_t _Idx, class _OtherExtents, class... _DynamicValues)
  _LIBCUDACXX_REQUIRES((_Idx < __rank_) _LIBCUDACXX_AND(static_extent(_Idx) != dynamic_extent))
  _LIBCUDACXX_INLINE_VISIBILITY constexpr _Values __construct_vals_from_extents(
    integral_constant<size_t, _DynCount>,
    integral_constant<size_t, _Idx>,
    const _OtherExtents& __exts,
    _DynamicValues... __dynamic_values) noexcept
  {
    return __construct_vals_from_extents(
      integral_constant<size_t, _DynCount>(), integral_constant<size_t, _Idx + 1>(), __exts, __dynamic_values...);
  }

  _LIBCUDACXX_TEMPLATE(size_t _DynCount, size_t _Idx, class _OtherExtents, class... _DynamicValues)
  _LIBCUDACXX_REQUIRES((_Idx == __rank_) && (_DynCount == __rank_dynamic_))
  _LIBCUDACXX_INLINE_VISIBILITY constexpr _Values __construct_vals_from_extents(
    integral_constant<size_t, _DynCount>,
    integral_constant<size_t, _Idx>,
    const _OtherExtents&,
    _DynamicValues... __dynamic_values) noexcept
  {
    return _Values{static_cast<index_type>(__dynamic_values)...};
  }

  template <class _OtherIndexType>
  static constexpr bool __potentially_narrowing = __mdspan_detail::__potentially_narrowing<index_type, _OtherIndexType>;

  _LIBCUDACXX_TEMPLATE(class _OtherIndexType, size_t... _OtherExtents)
  _LIBCUDACXX_REQUIRES((rank() > 0) _LIBCUDACXX_AND __potentially_narrowing<_OtherIndexType>)
  _LIBCUDACXX_INLINE_VISIBILITY constexpr extents(__extent_delegate_tag,
                                                  const extents<_OtherIndexType, _OtherExtents...>& __other) noexcept
      : __vals_(__construct_vals_from_extents(integral_constant<size_t, 0>(), integral_constant<size_t, 0>(), __other))
  {
    for (size_t __r = 0; __r < rank(); __r++)
    {
      // Not catching this could lead to out of bounds errors later
      // e.g. dextents<char,1>> e(dextents<unsigned,1>(200)) leads to an extent of -56 on e
      _LIBCUDACXX_ASSERT_VALID_ELEMENT_ACCESS(
        __mdspan_detail::__is_representable_as<index_type>(__other.extent(__r)),
        "extents ctor: arguments must be representable as index_type and nonnegative");
      // Not catching this could lead to out of bounds errors later
      // e.g. mdspan<int, extents<int, 10>> m = mdspan<int, dextents<int, 1>>(new int[5], 5);
      // Right-hand-side construction was ok, but m now thinks its range is 10 not 5
      _LIBCUDACXX_ASSERT_VALID_ELEMENT_ACCESS(
        (_Values::__static_value(__r) == dynamic_extent)
          || (static_cast<index_type>(__other.extent(__r)) == static_cast<index_type>(_Values::__static_value(__r))),
        "extents construction: mismatch of provided arguments with static extents.");
    }
  }

  _LIBCUDACXX_TEMPLATE(class _OtherIndexType, size_t... _OtherExtents)
  _LIBCUDACXX_REQUIRES((rank() > 0) _LIBCUDACXX_AND(!__potentially_narrowing<_OtherIndexType>))
  _LIBCUDACXX_INLINE_VISIBILITY constexpr extents(__extent_delegate_tag,
                                                  const extents<_OtherIndexType, _OtherExtents...>& __other) noexcept
      : __vals_(__construct_vals_from_extents(integral_constant<size_t, 0>(), integral_constant<size_t, 0>(), __other))
  {
    for (size_t __r = 0; __r < rank(); __r++)
    {
      // Not catching this could lead to out of bounds errors later
      // e.g. mdspan<int, extents<int, 10>> m = mdspan<int, dextents<int, 1>>(new int[5], 5);
      // Right-hand-side construction was ok, but m now thinks its range is 10 not 5
      _LIBCUDACXX_ASSERT_VALID_ELEMENT_ACCESS(
        (_Values::__static_value(__r) == dynamic_extent)
          || (static_cast<index_type>(__other.extent(__r)) == static_cast<index_type>(_Values::__static_value(__r))),
        "extents construction: mismatch of provided arguments with static extents.");
    }
  }

  _LIBCUDACXX_TEMPLATE(class _OtherIndexType, size_t... _OtherExtents)
  _LIBCUDACXX_REQUIRES((rank() == 0))
  _LIBCUDACXX_INLINE_VISIBILITY constexpr extents(__extent_delegate_tag,
                                                  const extents<_OtherIndexType, _OtherExtents...>& __other) noexcept
  {}

public:
  // Converting constructor from other extents specializations
  template <class _OtherIndexType, size_t... _OtherExtents>
  static constexpr bool __is_explicit_conversion =
    _CCCL_FOLD_OR(((_Extents != dynamic_extent) && (_OtherExtents == dynamic_extent)))
    || __potentially_narrowing<_OtherIndexType>;

  _LIBCUDACXX_TEMPLATE(class _OtherIndexType, size_t... _OtherExtents)
  _LIBCUDACXX_REQUIRES((sizeof...(_OtherExtents) == sizeof...(_Extents)) _LIBCUDACXX_AND _CCCL_FOLD_AND(
    (_OtherExtents == dynamic_extent || _Extents == dynamic_extent || _OtherExtents == _Extents))
                         _LIBCUDACXX_AND(!__is_explicit_conversion<_OtherIndexType, _OtherExtents...>))
  _LIBCUDACXX_INLINE_VISIBILITY constexpr extents(const extents<_OtherIndexType, _OtherExtents...>& __other) noexcept
      : extents(__extent_delegate_tag{}, __other)
  {}

  _LIBCUDACXX_TEMPLATE(class _OtherIndexType, size_t... _OtherExtents)
  _LIBCUDACXX_REQUIRES((sizeof...(_OtherExtents) == sizeof...(_Extents)) _LIBCUDACXX_AND _CCCL_FOLD_AND(
    (_OtherExtents == dynamic_extent || _Extents == dynamic_extent || _OtherExtents == _Extents))
                         _LIBCUDACXX_AND __is_explicit_conversion<_OtherIndexType, _OtherExtents...>)
  _LIBCUDACXX_INLINE_VISIBILITY explicit constexpr extents(
    const extents<_OtherIndexType, _OtherExtents...>& __other) noexcept
      : extents(__extent_delegate_tag{}, __other)
  {}

  // Comparison operator
  template <class _OtherIndexType, size_t... _OtherExtents>
  _CCCL_NODISCARD_FRIEND _LIBCUDACXX_INLINE_VISIBILITY constexpr auto
  operator==(const extents& __lhs, const extents<_OtherIndexType, _OtherExtents...>& __rhs) noexcept
    _LIBCUDACXX_TRAILING_REQUIRES(bool)((rank() != sizeof...(_OtherExtents)))
  {
    return false;
  }

  template <class _OtherIndexType, size_t... _OtherExtents>
  _CCCL_NODISCARD_FRIEND _LIBCUDACXX_INLINE_VISIBILITY constexpr auto
  operator==(const extents& __lhs, const extents<_OtherIndexType, _OtherExtents...>& __rhs) noexcept
    _LIBCUDACXX_TRAILING_REQUIRES(bool)((rank() == sizeof...(_OtherExtents) && rank() == 0))
  {
    return true;
  }

  template <class _OtherIndexType, size_t... _OtherExtents>
  _CCCL_NODISCARD_FRIEND _LIBCUDACXX_INLINE_VISIBILITY constexpr auto
  operator==(const extents& __lhs, const extents<_OtherIndexType, _OtherExtents...>& __rhs) noexcept
    _LIBCUDACXX_TRAILING_REQUIRES(bool)((rank() == sizeof...(_OtherExtents) && rank() != 0))
  {
    for (rank_type __r = 0; __r < __rank_; __r++)
    {
      // avoid warning when comparing signed and unsigner integers and pick the wider of two types
      using _CommonType = common_type_t<index_type, _OtherIndexType>;
      if (static_cast<_CommonType>(__lhs.extent(__r)) != static_cast<_CommonType>(__rhs.extent(__r)))
      {
        return false;
      }
    }
    return true;
  }

#  if _CCCL_STD_VER <= 2017
  template <class _OtherIndexType, size_t... _OtherExtents>
  _CCCL_NODISCARD_FRIEND _LIBCUDACXX_INLINE_VISIBILITY constexpr bool
  operator!=(const extents& __lhs, const extents<_OtherIndexType, _OtherExtents...>& __rhs) noexcept
  {
    return !(__lhs == __rhs);
  }
#  endif // _CCCL_STD_VER <= 2017
};

// Recursive helper classes to implement dextents alias for extents
namespace __mdspan_detail
{

template <class _IndexType, size_t _Rank, class _Extents = extents<_IndexType>>
struct __make_dextents;

template <class _IndexType, size_t _Rank, class _Extents = extents<_IndexType>>
using __make_dextents_t = typename __make_dextents<_IndexType, _Rank, _Extents>::type;

template <class _IndexType, size_t _Rank, size_t... _ExtentsPack>
struct __make_dextents<_IndexType, _Rank, extents<_IndexType, _ExtentsPack...>>
{
  using type = __make_dextents_t<_IndexType, _Rank - 1, extents<_IndexType, dynamic_extent, _ExtentsPack...>>;
};

template <class _IndexType, size_t... _ExtentsPack>
struct __make_dextents<_IndexType, 0, extents<_IndexType, _ExtentsPack...>>
{
  using type = extents<_IndexType, _ExtentsPack...>;
};

} // end namespace __mdspan_detail

// [mdspan.extents.dextents], alias template
template <class _IndexType, size_t _Rank>
using dextents = __mdspan_detail::__make_dextents_t<_IndexType, _Rank>;

#  if _CCCL_STD_VER >= 2017
// nvcc cannot handle type conversions without this workaround
struct __to_dynamic_extent
{
  template <class>
  static constexpr size_t value = dynamic_extent;
};

// Deduction guide for extents
template <class... _IndexTypes>
_CCCL_HOST_DEVICE extents(_IndexTypes...) -> extents<size_t, __to_dynamic_extent::template value<_IndexTypes>...>;
#  endif // _CCCL_STD_VER >= 2017

namespace __mdspan_detail
{

template <class _IndexType, size_t... _ExtentsPack>
struct __is_extents<extents<_IndexType, _ExtentsPack...>> : true_type
{};

// Function to check whether a set of indices are a multidimensional
// index into extents. This is a word of power in the C++ standard
// requiring that the indices are larger than 0 and smaller than
// the respective extents.

_LIBCUDACXX_TEMPLATE(class _IndexType, class _From)
_LIBCUDACXX_REQUIRES(integral<_IndexType> _LIBCUDACXX_AND integral<_From> _LIBCUDACXX_AND _CCCL_TRAIT(is_signed, _From))
_LIBCUDACXX_INLINE_VISIBILITY constexpr bool __is_index_in_extent(_IndexType __extent, _From __value)
{
  if (__value < 0)
  {
    return false;
  }
  using _Tp = common_type_t<_IndexType, _From>;
  return static_cast<_Tp>(__value) < static_cast<_Tp>(__extent);
}

_LIBCUDACXX_TEMPLATE(class _IndexType, class _From)
_LIBCUDACXX_REQUIRES(
  integral<_IndexType> _LIBCUDACXX_AND integral<_From> _LIBCUDACXX_AND(!_CCCL_TRAIT(is_signed, _From)))
_LIBCUDACXX_INLINE_VISIBILITY constexpr bool __is_index_in_extent(_IndexType __extent, _From __value)
{
  using _Tp = common_type_t<_IndexType, _From>;
  return static_cast<_Tp>(__value) < static_cast<_Tp>(__extent);
}

_LIBCUDACXX_TEMPLATE(class _IndexType, class _From)
_LIBCUDACXX_REQUIRES(integral<_IndexType> _LIBCUDACXX_AND(!integral<_From>)
                       _LIBCUDACXX_AND _CCCL_TRAIT(is_signed, _From))
_LIBCUDACXX_INLINE_VISIBILITY constexpr bool __is_index_in_extent(_IndexType __extent, _From __value)
{
  if (static_cast<_IndexType>(__value) < 0)
  {
    return false;
  }
  return static_cast<_IndexType>(__value) < __extent;
}

_LIBCUDACXX_TEMPLATE(class _IndexType, class _From)
_LIBCUDACXX_REQUIRES(integral<_IndexType> _LIBCUDACXX_AND(!integral<_From>)
                       _LIBCUDACXX_AND(!_CCCL_TRAIT(is_signed, _From)))
_LIBCUDACXX_INLINE_VISIBILITY constexpr bool __is_index_in_extent(_IndexType __extent, _From __value)
{
  return static_cast<_IndexType>(__value) < __extent;
}

template <size_t... _Idxs, class _Extents, class... _From>
_LIBCUDACXX_INLINE_VISIBILITY constexpr bool
__is_multidimensional_index_in_impl(index_sequence<_Idxs...>, const _Extents& __ext, _From... __values)
{
  return _CCCL_FOLD_AND(__mdspan_detail::__is_index_in_extent(__ext.extent(_Idxs), __values));
}

template <class _Extents, class... _From>
_LIBCUDACXX_INLINE_VISIBILITY constexpr bool __is_multidimensional_index_in(const _Extents& __ext, _From... __values)
{
  return __mdspan_detail::__is_multidimensional_index_in_impl(
    make_index_sequence<_Extents::rank()>(), __ext, __values...);
}

} // namespace __mdspan_detail

_LIBCUDACXX_END_NAMESPACE_STD

#endif // _CCCL_STD_VER >= 2014

_CCCL_POP_MACROS

#endif // _LIBCUDACXX___MDSPAN_EXTENTS_H
