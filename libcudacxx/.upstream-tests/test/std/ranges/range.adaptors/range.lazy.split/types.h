...
struct CopyableView : cuda::std::ranges::view_base {
  ...
  constexpr CopyableView(cuda::std::string_view v) : view_(v) {}
...
static_assert( cuda::std::ranges::forward_range<CopyableView>);
static_assert( cuda::std::ranges::forward_range<const CopyableView>);
static_assert( cuda::std::ranges::view<CopyableView>);
static_assert( cuda::std::is_copy_constructible_v<CopyableView>);
...
struct ForwardView : cuda::std::ranges::view_base {
  ...
  constexpr ForwardView(cuda::std::string_view v) : view_(v) {}
...
static_assert( cuda::std::ranges::forward_range<const ForwardView>);
static_assert( cuda::std::ranges::view<ForwardView>);
static_assert(!cuda::std::is_copy_constructible_v<ForwardView>);
static_assert( cuda::std::is_move_constructible_v<ForwardView>);

// ForwardDiffView

// Iterator types differ based on constness of this class.
struct ForwardDiffView : cuda::std::ranges::view_base {
  cuda::std::string buffer_;
  constexpr explicit ForwardDiffView() = default;
  constexpr ForwardDiffView(const char* ptr) : ForwardDiffView(cuda::std::string_view(ptr)) {}
  constexpr ForwardDiffView(cuda::std::string_view v) {
...
static_assert( cuda::std::ranges::forward_range<const ForwardView>);
static_assert( cuda::std::ranges::view<ForwardView>);
static_assert(!cuda::std::same_as<cuda::std::ranges::iterator_t<ForwardDiffView>, cuda::std::ranges::iterator_t<const ForwardDiffView>>);
...
static_assert(!cuda::std::forward_iterator<almost_forward_iterator<int*>>);
static_assert( cuda::std::input_iterator<almost_forward_iterator<int*>>);

struct ForwardOnlyIfNonConstView : cuda::std::ranges::view_base {
  cuda::std::string_view view_;

  constexpr explicit ForwardOnlyIfNonConstView() = default;
  constexpr ForwardOnlyIfNonConstView(const char* ptr) : view_(ptr) {}
  constexpr ForwardOnlyIfNonConstView(cuda::std::string_view v) : view_(v) {}
...
static_assert( cuda::std::ranges::forward_range<ForwardOnlyIfNonConstView>);
static_assert(!cuda::std::ranges::forward_range<const ForwardOnlyIfNonConstView>);
static_assert( cuda::std::ranges::view<ForwardOnlyIfNonConstView>);
...
struct InputView : cuda::std::ranges::view_base {
  ...
  constexpr InputView(const char* s) : InputView(cuda::std::string_view(s)) {}
  constexpr InputView(cuda::std::string_view v) {
    // Workaround https://github.com/llvm/llvm-project/issues/55867
    buffer_ = v;
  }
...
static_assert(cuda::std::ranges::input_range<InputView>);
static_assert(cuda::std::ranges::input_range<const InputView>);
static_assert(cuda::std::ranges::view<InputView>);

// ForwardTinyView

struct ForwardTinyView : cuda::std::ranges::view_base {
  char c_[1] = {};
...
static_assert(cuda::std::ranges::forward_range<ForwardTinyView>);
static_assert(cuda::std::ranges::view<ForwardTinyView>);
LIBCPP_STATIC_ASSERT(cuda::std::ranges::__tiny_range<ForwardTinyView>);

// Aliases

using SplitViewCopyable = cuda::std::ranges::lazy_split_view<CopyableView, CopyableView>;
using OuterIterCopyable = cuda::std::ranges::iterator_t<SplitViewCopyable>;
using ValueTypeCopyable = OuterIterCopyable::value_type;
using InnerIterCopyable = cuda::std::ranges::iterator_t<ValueTypeCopyable>;
using BaseIterCopyable = cuda::std::ranges::iterator_t<CopyableView>;

using SplitViewForward = cuda::std::ranges::lazy_split_view<ForwardView, ForwardView>;
using OuterIterForward = cuda::std::ranges::iterator_t<SplitViewForward>;
using ValueTypeForward = OuterIterForward::value_type;
using InnerIterForward = cuda::std::ranges::iterator_t<ValueTypeForward>;
using BaseIterForward = cuda::std::ranges::iterator_t<ForwardView>;

using SplitViewInput = cuda::std::ranges::lazy_split_view<InputView, ForwardTinyView>;
using OuterIterInput = cuda::std::ranges::iterator_t<SplitViewInput>;
using ValueTypeInput = OuterIterInput::value_type;
using InnerIterInput = cuda::std::ranges::iterator_t<ValueTypeInput>;
using BaseIterInput = cuda::std::ranges::iterator_t<InputView>;

using SplitViewDiff = cuda::std::ranges::lazy_split_view<ForwardDiffView, ForwardDiffView>;
using OuterIterConst = decltype(cuda::std::declval<const SplitViewDiff>().begin());
using OuterIterNonConst = decltype(cuda::std::declval<SplitViewDiff>().begin());
static_assert(!cuda::std::same_as<OuterIterConst, OuterIterNonConst>);
using InnerIterConst = decltype((*cuda::std::declval<OuterIterConst>()).begin());
using InnerIterNonConst = decltype((*cuda::std::declval<OuterIterNonConst>()).begin());
static_assert(!cuda::std::same_as<InnerIterConst, InnerIterNonConst>);
