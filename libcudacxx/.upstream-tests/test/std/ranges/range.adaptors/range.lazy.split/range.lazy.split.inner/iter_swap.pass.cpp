...
constexpr bool test() {
  // Can use `iter_swap` with `inner-iterator`; `View` is a forward range.
  {
    // Non-const iterator.
    {
      ...
      auto i2 = i1++;
      static_assert(cuda::std::is_void_v<decltype(iter_swap(i1, i2))>);
      assert(*i1 == 'b');
      assert(*i2 == 'a');
    ...
    // Const iterator.
    {
      ...
      const auto i2 = i1++;
      static_assert(cuda::std::is_void_v<decltype(iter_swap(i1, i2))>);
      static_assert(cuda::std::is_void_v<decltype(iter_swap(i2, i2))>);
      assert(*i1 == 'b');
      assert(*i2 == 'a');
  ...
  {
    {
      ...
      ASSERT_NOT_NOEXCEPT(cuda::std::ranges::iter_swap(
          cuda::std::declval<adl::MaybeNoexceptIterator<false>>(),
          cuda::std::declval<adl::MaybeNoexceptIterator<false>>()));
...

    {
      ...
      ASSERT_NOEXCEPT(cuda::std::ranges::iter_swap(
          cuda::std::declval<adl::MaybeNoexceptIterator<true>>(),
          cuda::std::declval<adl::MaybeNoexceptIterator<true>>()));
