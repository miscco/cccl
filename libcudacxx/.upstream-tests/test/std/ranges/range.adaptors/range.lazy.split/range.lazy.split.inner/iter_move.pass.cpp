...
constexpr bool test() {
  // Can use `iter_move` with `inner-iterator`; `View` is a forward range.
  {
    ...
    // Non-const iterator.
    {
      ...
      static_assert(cuda::std::same_as<decltype(iter_move(i)), const char &&>);
  ...
  // Can use `iter_move` with `inner-iterator`, `View` is an input range.
  {
    ...
    {
      auto i = segment.begin();
      static_assert(cuda::std::same_as<decltype(iter_move(i)), char &&>);
      assert(iter_move(i) == 'a');
    }
  ...
  // Ensure the `iter_move` customization point is being used.
  {
    ...
    cuda::std::ranges::lazy_split_view<adl::View<>, adl::View<>> v(input, adl::View<>());
  ...
  {
    {
      using ThrowingSplitView = cuda::std::ranges::lazy_split_view<adl::View<false>, adl::View<false>>;
      using ThrowingValueType = cuda::std::ranges::iterator_t<ThrowingSplitView>::value_type;
      using ThrowingIter = cuda::std::ranges::iterator_t<ThrowingValueType>;
      ASSERT_NOT_NOEXCEPT(cuda::std::ranges::iter_move(cuda::std::declval<adl::MaybeNoexceptIterator<false>>()));
      ASSERT_NOT_NOEXCEPT(iter_move(cuda::std::declval<ThrowingIter>()));
    }

    {
      using NoexceptSplitView = cuda::std::ranges::lazy_split_view<adl::View<true>, adl::View<true>>;
      using NoexceptValueType = cuda::std::ranges::iterator_t<NoexceptSplitView>::value_type;
      using NoexceptIter = cuda::std::ranges::iterator_t<NoexceptValueType>;
      ASSERT_NOEXCEPT(cuda::std::ranges::iter_move(cuda::std::declval<adl::MaybeNoexceptIterator<true>>()));
