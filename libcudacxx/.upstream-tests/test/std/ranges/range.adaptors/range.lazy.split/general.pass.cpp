...
// Unlike `cuda::std::string_view`, it copies the given string into an
// internal buffer and can work with non-contiguous inputs.
template <class Char>
class BasicSmallString {
  cuda::std::basic_string<Char> buffer_{};

public:
  constexpr BasicSmallString(cuda::std::basic_string_view<Char> v) : buffer_(v) {}
...
template <cuda::std::ranges::view View, cuda::std::ranges::range Expected>
constexpr bool is_equal(View& view, const Expected& expected) {
  using Char = cuda::std::ranges::range_value_t<cuda::std::ranges::range_value_t<View>>;
...
constexpr bool test_function_call(T&& input, Separator&& separator, cuda::std::array<U, M> expected) {
  cuda::std::ranges::lazy_split_view v(input, separator);
  return is_equal(v, expected);
}

template <class T, class Separator, class U, size_t M>
constexpr bool test_with_piping(T&& input, Separator&& separator, cuda::std::array<U, M> expected) {
  auto expected_it = expected.begin();
  for (auto e : input | cuda::std::ranges::views::lazy_split(separator)) {
    if (expected_it == expected.end())
      return false;
    if (!cuda::std::ranges::equal(e, *expected_it))
      return false;
...
constexpr bool test_l_r_values() {
  ...
  // Both lvalues and rvalues can be used as input.
  {
    // Lvalues.
    {
      auto input = "abc"sv;
      auto sep = " "sv;
      [[maybe_unused]] cuda::std::ranges::lazy_split_view v(input, sep);
    }
    ...
    // Rvalues.
    {
      auto input = "abc"sv;
      auto sep = " "sv;
      [[maybe_unused]] cuda::std::ranges::lazy_split_view v(cuda::std::move(input), cuda::std::move(sep));
    }
...
constexpr bool test_string_literal_separator() {
  ...
  {
    cuda::std::ranges::lazy_split_view v("abc def"sv, ' ');
    assert(is_equal(v, cuda::std::array{"abc"sv, "def"sv}));
  }
  ...
  {
    cuda::std::ranges::lazy_split_view v("abc def"sv, " ");
    assert(is_equal(v, cuda::std::array{"abc def"sv}));
  }
  ...
  {
    cuda::std::ranges::lazy_split_view v("abc \0def"sv, " ");
    assert(is_equal(v, cuda::std::array{"abc"sv, "def"sv}));
  }
...
// below).
template <class T>
constexpr cuda::std::string_view sv(T&& str) {
  return cuda::std::string_view(str);
};

template <class T, class Separator, class U, size_t M>
constexpr void test_one(T&& input, Separator&& separator, cuda::std::array<U, M> expected) {
  ...
  if constexpr (cuda::std::is_same_v<cuda::std::remove_reference_t<Separator>, char>) {
...
constexpr bool test_string_literals() {
  ...
  // the original range).
  {
    cuda::std::array expected = {"abc"sv, cuda::std::string_view("def", sizeof("def"))};
  ...
  // Empty string.
  {
    ...
    cuda::std::array expected = {cuda::std::string_view("", 1)};
  ...
  // are treated differently due to the presence of an implicit `\0` in the latter.
  {
    ...
    cuda::std::array expected_unsplit = {cuda::std::string_view(input, sizeof(input))};
    cuda::std::array expected_split = {"abc"sv, cuda::std::string_view("def", sizeof("def"))};
  ...
  {
    auto empty_sep = ""sv;
    cuda::std::array expected = {"a"sv, "b"sv, "c"sv, "\0"sv};
...
bool test_nontrivial_characters() {
  ...
  using Map = cuda::std::map<cuda::std::string, int>;
  using Vec = cuda::std::vector<Map>;
  ...
  cuda::std::ranges::lazy_split_view v(Vec{m1, m2, sep, m3}, sep);
...
constexpr bool main_test() {
  ...
  // One separator.
  {
    cuda::std::array expected = {"abc"sv, "def"sv};
  ...
  {
    cuda::std::array expected = {"abc"sv, ""sv, ""sv, ""sv, "def"sv};
    test_one("abc    def"sv, short_sep, expected);
  ...
  // Trailing separator.
  {
    cuda::std::array expected = {"abc"sv, "def"sv, ""sv};
    test_one("abc def "sv, short_sep, expected);
  ...
  // Leading separator.
  {
    cuda::std::array expected = {""sv, "abc"sv, "def"sv};
    test_one(" abc def"sv, short_sep, expected);
  ...
  // No separator.
  {
    cuda::std::array expected = {"abc"sv};
  ...
  {
    cuda::std::array expected = {""sv, ""sv, ""sv, ""sv};
    test_one("   "sv, short_sep, expected);
  ...
  // The separator and the string use the same character only.
  {
    ...
    cuda::std::array expected = {""sv, "aa"sv};
    test_one("aaaaa"sv, overlapping_sep, expected);
  }

  // Many redundant separators.
  {
    cuda::std::array expected = {""sv, ""sv, "abc"sv, ""sv, ""sv, "def"sv, ""sv, ""sv};
    test_one("  abc   def  "sv, short_sep, expected);
  ...
  {
    cuda::std::array expected = {""sv, "a"sv, "b"sv, "c"sv, ""sv};
  ...
  // Overlap between the separator and the string (see https://wg21.link/lwg3505).
  {
    ...
    cuda::std::array expected = {"a"sv, "aa"sv, ""sv, "b"sv};
    test_one("aabaaababb"sv, overlapping_sep, expected);
  }

  // Empty input.
  {
    cuda::std::array<cuda::std::string_view, 0> expected = {};
    test_one(""sv, short_sep, expected);
  ...
  {
    auto empty_sep = ""sv;
    cuda::std::array expected = {"a"sv, "b"sv, "c"sv};
  ...
  {
    // `char`.
    test_function_call("abc def", ' ', cuda::std::array{"abc", "def"});
#ifndef TEST_HAS_NO_WIDE_CHARACTERS
    // `wchar_t`.
    test_function_call(L"abc def", L' ', cuda::std::array{L"abc", L"def"});
#endif
    // `char8_t`.
    test_function_call(u8"abc def", u8' ', cuda::std::array{u8"abc", u8"def"});
    // `char16_t`.
    test_function_call(u"abc def", u' ', cuda::std::array{u"abc", u"def"});
    // `char32_t`.
    test_function_call(U"abc def", U' ', cuda::std::array{U"abc", U"def"});
  }
  ...
  // Non-character input.
  {
    cuda::std::array expected = {cuda::std::array{1, 2, 3}, cuda::std::array{4, 5, 6}};
    test_one(cuda::std::array{1, 2, 3, 0, 4, 5, 6}, 0, expected);
    test_one(cuda::std::array{1, 2, 3, 0, 0, 0, 4, 5, 6}, cuda::std::array{0, 0, 0}, expected);
  }
