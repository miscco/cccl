...
struct MoveOnlyView : cuda::std::ranges::view_base {
  ...
  constexpr MoveOnlyView(cuda::std::string_view v) : view_(v) {}
...
struct ViewWithInitTracking : cuda::std::ranges::view_base {
  ...
  cuda::std::string_view v_;
  InitializedBy initialized_by = InitializedBy::Invalid;
  constexpr ViewWithInitTracking(cuda::std::string_view v) : v_(v) {}
...
template <class View>
concept CanCallBase = requires(View v) { cuda::std::forward<View>(v).base(); };

static_assert( CanCallBase<cuda::std::ranges::lazy_split_view<MoveOnlyView, ForwardView>&&>);
static_assert(!CanCallBase<cuda::std::ranges::lazy_split_view<MoveOnlyView, ForwardView>&>);
static_assert(!CanCallBase<cuda::std::ranges::lazy_split_view<MoveOnlyView, ForwardView> const &>);
static_assert(!CanCallBase<cuda::std::ranges::lazy_split_view<MoveOnlyView, ForwardView> const &&>);
...
constexpr bool test() {
  ...
  // Copyable input -- both lvalue and rvalue overloads of `base` are available.
  {
    ...
    {
      View str{"abc def"};
      cuda::std::ranges::lazy_split_view<View, cuda::std::string_view> v(str, " ");

      cuda::std::same_as<View> decltype(auto) result = v.base();
      assert(result == str);
    ...
    {
      View str{"abc def"};
      const cuda::std::ranges::lazy_split_view<View, cuda::std::string_view> v(str, " ");
    ...
    // Non-const rvalue.
    {
      ...
      cuda::std::same_as<View> decltype(auto) result = cuda::std::move(v).base();
      assert(result == str);
  ...
  {
    cuda::std::ranges::lazy_split_view<MoveOnlyView, ForwardView> v;
    assert(cuda::std::move(v).base() == MoveOnlyView());
  }
