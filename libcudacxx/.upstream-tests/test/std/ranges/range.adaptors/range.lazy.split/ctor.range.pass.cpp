...
static_assert( cuda::std::ranges::forward_range<RangeWithCounting>);
static_assert(!cuda::std::ranges::view<RangeWithCounting>);

struct StrView : cuda::std::ranges::view_base {
  cuda::std::string_view buffer_;
  constexpr explicit StrView() = default;
  constexpr StrView(const char* ptr) : buffer_(ptr) {}
  // Intentionally don't forward to range constructor for cuda::std::string_view since
...
static_assert( cuda::std::ranges::random_access_range<StrView>);
static_assert( cuda::std::ranges::view<StrView>);
static_assert( cuda::std::is_copy_constructible_v<StrView>);

constexpr bool test() {
  {
    using V = cuda::std::ranges::lazy_split_view<StrView, StrView>;

    // Calling the constructor with `(cuda::std::string, range_value_t)`.
    {
      cuda::std::string input;
      V v(input, ' ');
      assert(v.base() == input);
    }
    ...
    struct Empty {};
    static_assert(!cuda::std::is_constructible_v<V, Empty, cuda::std::string_view>);
    static_assert(!cuda::std::is_constructible_v<V, cuda::std::string_view, Empty>);
  }
  ...
  // Make sure the arguments are moved, not copied.
  {
    ...
    using Pattern = cuda::std::ranges::single_view<Element>;

    // Arguments are lvalues.
    {
      using View = cuda::std::ranges::ref_view<Range>;
      ...
      cuda::std::ranges::lazy_split_view<View, Pattern> v(range, element);
    ...
    {
      using View = cuda::std::ranges::owning_view<Range>;
