...
1. the :ref:`cuda::get_property <libcudacxx-extended-api-memory-resources-properties>`
   infrastructure to tag a user defined type with a given property;
2. the :ref:`cuda::mr::{async}_resource <libcudacxx-extended-api-memory-resources-resource>`
   and :ref:`cuda::mr::{async}_resource_with <libcudacxx-extended-api-memory-resources-resource>`
   concepts that provide proper constraints for arbitrary memory resources;
3. the :ref:`cuda::mr::{async}_resource_ref <libcudacxx-extended-api-memory-resources-resource-ref>`
   is a type-erased memory resource wrapper that enables consumers to specify properties of resources that they expect.
...
libcu++ provides :ref:`cuda::stream_ref <libcudacxx-extended-api-stream-ref>` as a wrapper around
