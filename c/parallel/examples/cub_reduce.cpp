...
#define CU_CHECK(call)                                                                                       \
  ...
  {                                                                                                          \
    ...
    {                                                                                                        \
      ...
      std::cerr << "CU error at " << __FILE__ << ":" << __LINE__ << ": " << (str ? str : "unknown") << "\n"; \
      return 1;                                                                                              \
    }                                                                                                        \
  } while (0)
...
int main()
{
  ...
  auto pch_dir    = std::filesystem::temp_directory_path() / "hostjit_pch";
  ...
  std::cout << "PCH cache: " << pch_dir.string() << (pch_cached ? " (hit)" : " (miss, will generate)") << "\n\n";
  ...
  // ── Sum reduction ────────────────────────────────────────────────
  {
    ...
    std::cout << "Build time (ms): " << std::chrono::duration<double, std::milli>(t1 - t0).count() << "\n";

    size_t temp_bytes = 0;
    CU_CHECK(cccl_device_reduce(build, nullptr, &temp_bytes, input_it, output_it, N, op, init, nullptr));
    void* d_temp = nullptr;
    CUDA_CHECK(cudaMalloc(&d_temp, temp_bytes));
    CU_CHECK(cccl_device_reduce(build, d_temp, &temp_bytes, input_it, output_it, N, op, init, nullptr));
